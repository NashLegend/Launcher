...
/**
 * “所有程序”界面的tabhost，事实上它也包含了content。换句话说，它就是实际上的“所有程序和widget”界面
 * 
 */
public class AppsCustomizeTabHost extends TabHost implements
	...
	protected void onFinishInflate() {
		...
		// (that we change the content filter on)
			...
			public void run() {
				...
				for (int i = visiblePages.size() - 1; i >= 0; i--) {
					...
					if (child instanceof PagedViewCellLayout) {
						...
						((PagedViewGridLayout) child)
								.resetChildrenOnKeyListeners();
					}
					PagedViewWidget
							.setDeletePreviewsWhenDetachedFromWindow(false);
					mAppsCustomizePane.removeView(child);
					PagedViewWidget
							.setDeletePreviewsWhenDetachedFromWindow(true);
					mAnimationBuffer.setAlpha(1f);
					mAnimationBuffer.setVisibility(View.VISIBLE);
					LayoutParams p = new FrameLayout.LayoutParams(
							child.getMeasuredWidth(), child.getMeasuredHeight());
					p.setMargins((int) child.getLeft(), (int) child.getTop(),
							0, 0);
				...
				ObjectAnimator outAnim = LauncherAnimUtils.ofFloat(
				...
				ObjectAnimator inAnim = LauncherAnimUtils.ofFloat(
				...
				final AnimatorSet animSet = LauncherAnimUtils
						.createAnimatorSet();
	...
	public void onLauncherTransitionPrepare(Launcher l, boolean animated,
			boolean toWorkspace) {
		mAppsCustomizePane
				.onLauncherTransitionPrepare(l, animated, toWorkspace);
		mInTransition = true;
		...
		if (toWorkspace) {
			...
			// Stop the scrolling indicator - we don't want All Apps to be
			// invalidating itself
			// during the transition, especially since it has a hardware layer
			// set on it
	...
	public void onWindowVisible() {
		if (getVisibility() == VISIBLE) {
			...
			mAppsCustomizePane.loadAssociatedPages(
					mAppsCustomizePane.getCurrentPage(), true);
			mAppsCustomizePane.loadAssociatedPages(
					mAppsCustomizePane.getCurrentPage(), false);
		}
	}
