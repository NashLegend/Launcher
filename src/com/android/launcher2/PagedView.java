...
/**
 * 支持分页查看的workspace的一个抽象
 */
public abstract class PagedView extends ViewGroup implements
		ViewGroup.OnHierarchyChangeListener {
	private static final String TAG = "PagedView";
	private static final boolean DEBUG = false;
	protected static final int INVALID_PAGE = -1;

	// 达成fling所需最小的距离，防止一不留神滑页了
	...
	// 触摸滑动状态，没有状态，正在滑动，左滑条件达成，右滑条件达成
	...
	// 是否在左右滑动时改变相邻page的alpha
	protected boolean mFadeInAdjacentScreens = true;

	// 是否使用一个不同的slop参数判定是否换到一个新page(pagingTouchSlop = 2 * touchSlop)
	protected boolean mUsePagingTouchSlop = true;

	// 子类是否在computeScroll方法里立刻更新scrollX
	 ...
	 * 将当前页立即滚动到目标位置。用于显示“所有程序”的情况下，当重置前一个tab包含的page的滚动状态时，允许tab们共用相同page。
	 */
	...
	/**
	 * 当在“所有程序”和前台桌面间切换时，停止滚动，当其他动画完成时调用resumeScrolling
	 * 和updateCurrentPageScroll()以确保滚动完成并再次enable滚动
	 */
	void pauseScrolling() {
		mScroller.forceFinished(true);
	...
	void setCurrentPage(int currentPage) {
		...
		// 如果我们连默认值都改变了，那么不要加入任何像 mCurrentPage == currentPage这样的判断
		if (getChildCount() == 0) {
			return;
		}
	...
	// reuse it
	// 将这个功能加入到一个helper方法中以便SmoothPagedView可以重用它
	protected boolean computeScrollHelper() {
		if (mScroller.computeScrollOffset()) {
			// 如果滚动尚未完毕
			// 如果页面不需要滚动就别滚了（也就是页面的位置和当前正在计算的位置一样）
			...
			// 只有滚动结束时（rest状态）才执行滚动停止事件
			if (mTouchState == TOUCH_STATE_REST) {
				pageEndMoving();
			}

			// 页面改变时通知用户。不懂什么意思 TODO
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		 ...
		 * the same width. 高度允许自适应，但是宽度不允许，这是代码需要
		 */
		...
		// 调用setMeasuredDimension之前我们不能调用 getChildOffset/getRelativeChildOffset
	...
	public void setLayoutScale(float childrenScale) {
		...
		// 页面变小了，那么page offset就变了，所以要重新滚动一下。但是仍然要保持绝对XY坐标
	...
	/**
	 * 滚动时修改indicator位置前修改相信page的alpha
	 */
	...
	public void onChildViewAdded(View parent, View child) {
		// 确保当一个child被添加时，其大小alpha等要与当前滚动状态相符（比如在相信page里要变得透明并且变小）
	...
	/**
	 * 获取可见的page，不明，莫非是参与变换的几个page，range是两头的值? TODO
	 */
	...
	public void requestDisallowInterceptTouchEvent(boolean disallowIntercept) {
		if (disallowIntercept) {
			// 如果一个可滚动widget接管了touch事件，那么取消长按事件
	...
	/**
	 * page.如果一个tap可以触发flip到前一页的话,返回true
	 */
	...
	/**
	 * page.如果一个tap可以触发flip到下一页的话,返回true
	 */
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			/*
			 * 当前不是正在拖动的状态，而是要检查是否用户的手指已经动了足够远
			 */
			if (mActivePointerId != INVALID_POINTER) {
				determineScrollingStart(ev);
				break;
			}
			// 如果 mActivePointerId是INVALID_POINTER，
			// 那么我们没有接收到DOWN事件，所以要把第一次move事件当成DOWN，因些这里没有break
			// 当workspace变小的时候这就会发生
		}
	...
	protected String getCurrentPageDescription() {
		return String.format(
				getContext().getString(R.string.default_scroll_format),
