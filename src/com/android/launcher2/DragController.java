...
public class DragController {
	...
	/** 表示这次拖动是一个复制行为，比如在桌面上移动图标 */
	public static int DRAG_ACTION_MOVE = 0;

	/** 表示这次拖动是一个复制行为，比如从“所有程序”界面 放到桌面 */
	...
	/**
	 * 滑动到桌面左右边上的某区域时桌面就会滚动。就是这个区域
	 */
	private int mScrollZone;

	private DropTarget.DragObject mDragObject;

	/** 可以接收本次drag的view和listener */
	 ...
	 *            被拖动的view
	 * @param bmp
	 *            表示被拖动view和bitmap The bitmap that represents the view being
	 *            dragged
	 * @param source
	 *            发起拖动的对象 An object representing where the drag originated
	 * @param dragInfo
	 *            与被拖动对象关联的数据 The data associated with the object that is being
	 *            dragged
	 * @param dragAction
	 *            拖动的动作：移动或者复制 either {@link #DRAG_ACTION_MOVE} or
	 *            {@link #DRAG_ACTION_COPY}
	 * @param dragRegion
	 *            拖动对象bitmap的区域 Coordinates within the bitmap b for the position
	 ...
	 * 返回x或y在draglayer中间，那么返回x或y如果在外面 ，返回邻近的边
	 */
	...
	/**
	 * Call this from a drag source view. 由drag
	 * source调用。dragsource有可能是workspace，folder或者appPagedView
	 */
