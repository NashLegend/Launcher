...
public final class Launcher extends Activity implements View.OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// current orientation。
		// 在大的界面（sw600dp）上，我们想让屏幕自动旋转。（这时候全部加载并显示完了没？）
		unlockScreenOrientation(true);
	}

	/*
	 * 程序进入后台之前触发（比如接了Home键），在onPause之前调用
	 */
	...
	/**
	 * 找到所有需要的view并正确地配置它们，包括Launcher根view,dragLayer,workSpace,Qsb等等
	 */
	...
	/**
	 * 注册一些content observers。当前只注册了favorites observer以跟踪favorites applications。
	 * 什么是favorites applications？
	 */
		...
		if (mQsbDivider != null && mDockDivider != null) {
			...
			if (animated) {
				...
				mDividerAnimator.playTogether(
						LauncherAnimUtils.ofFloat(mQsbDivider, "alpha", 1f),
